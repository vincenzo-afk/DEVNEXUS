...
export default function PitchGenerator({ projectName, description, stack, onClose }: PitchGeneratorProps) {
  ...
  const fetchPitch = async () => {
    ...
    setError(null);
    try {
      const response = await generatePitch(projectName, description, stack);
      // Backend returns either direct PitchResult or wrapped in pitch/result
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-md" onClick={onClose}>
      <motion.div
        ...
        className="relative w-full max-w-3xl rounded-3xl border border-white/12 p-6 md:p-8 bg-[#0a0b18] overflow-hidden"
        style={{
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Elevator pitch for {projectName}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="max-h-[60vh] overflow-y-auto pr-1 space-y-5">
          {loading ? (
            ...
            <div className="p-4 rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-300 text-sm mb-4">
              <span className="font-semibold">⚠️ Note:</span> {error}
            </div>
          ) : null}
          ...
          {pitch && !loading && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-4">
                ...
                {/* Solution Section */}
                <div className="glass-card p-4 rounded-2xl border border-white/8 bg-white/2 hover:border-white/15 transition-all">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-green-400 mb-2 flex items-center gap-1.5">
              ...
              <div className="space-y-4">
                {/* Demo Flow Section */}
                <div className="glass-card p-4 rounded-2xl border border-white/8 bg-white/2 hover:border-white/15 transition-all h-fit">
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-white/10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 transition-all text-sm font-bold text-white shadow-lg shadow-indigo-600/30 disabled:opacity-50"
            >
              {copied ? (
                <><Check className="w-4 h-4" /> Copied!</>
              ) : (
                <><Copy className="w-4 h-4" /> Copy Pitch Script</>
              )}
            </button>
          </div>
        </div>
